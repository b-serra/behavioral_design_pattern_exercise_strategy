...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "percent", "bulk", "fixed", "composite"],  # Added "fixed"
        help="Strategy kind",
    )
    parser.add_argument(
        "--percent",
    ...
    parser.add_argument(
        "--threshold", type=int, default=0, help="Qty threshold for bulk/composite"
    )
    parser.add_argument(
        "--per-item-off",
        type=float,
    ...
    parser.add_argument(
        "--amount", type=float, default=0.0, help="Fixed amount discount for 'fixed'"
    )  # Added for fixed
    ...
    strat = choose_strategy(
        ...
        amount=args.amount,  # Pass amount for fixed
    )
